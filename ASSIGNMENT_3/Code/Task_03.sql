...
-- STEP 10: Automated benchmark summary (serial vs parallel) using EXPLAIN JSON
--------------------------------------------------------------------------------

-- Table to store benchmark runs
CREATE TABLE IF NOT EXISTS public.parallel_serial_benchmark (
    BenchmarkID SERIAL PRIMARY KEY,
    Mode VARCHAR(20) NOT NULL, -- Serial | Parallel
    Workers INT NOT NULL,
    QueryName VARCHAR(50) NOT NULL,
    ExecutionTime_ms DECIMAL(12,2) NOT NULL,
    PlanRows BIGINT,
    BuffersRead BIGINT,
    BuffersHit BIGINT,
...
-- Helper function: run target query, time it with clock_timestamp, and log metrics
DROP FUNCTION IF EXISTS public.log_benchmark(text, text, int);
CREATE OR REPLACE FUNCTION public.log_benchmark(p_mode text, p_query_name text, p_workers int)
RETURNS VOID AS $$
DECLARE
    t_start timestamp;
    t_end timestamp;
    ...
    rec RECORD;
BEGIN
    PERFORM set_config('max_parallel_workers_per_gather', p_workers::text, true);

    t_start := clock_timestamp();

    -- Execute the aggregate query and count returned rows
    FOR rec IN 
        SELECT 
            Type,
            Status,
    ...
    v_time_ms := EXTRACT(EPOCH FROM (t_end - t_start)) * 1000.0;

    INSERT INTO public.parallel_serial_benchmark (Mode, Workers, QueryName, ExecutionTime_ms, PlanRows, BuffersRead, BuffersHit)
    VALUES (p_mode, p_workers, p_query_name, v_time_ms, v_rows, NULL, NULL);
END;
$$ LANGUAGE plpgsql;

-- Run serial benchmark
TRUNCATE public.parallel_serial_benchmark;
SELECT public.log_benchmark('Serial', 'AggregateActivePolicies', 0);

-- Run parallel benchmark (4 workers)
SELECT public.log_benchmark('Parallel', 'AggregateActivePolicies', 4);

-- Display summary view of serial vs parallel
WITH summary AS (
    SELECT 
        MAX(CASE WHEN Mode = 'Serial' THEN ExecutionTime_ms END) AS serial_ms,
        MAX(CASE WHEN Mode = 'Parallel' THEN ExecutionTime_ms END) AS parallel_ms,
        MAX(CASE WHEN Mode = 'Serial' THEN Workers END) AS serial_workers,
        MAX(CASE WHEN Mode = 'Parallel' THEN Workers END) AS parallel_workers
    FROM public.parallel_serial_benchmark
    WHERE QueryName = 'AggregateActivePolicies'
)
SELECT 
    serial_ms,
    parallel_ms,
    CASE WHEN serial_ms > 0 THEN ROUND(((serial_ms - parallel_ms) / serial_ms) * 100, 2) ELSE NULL END AS improvement_pct,
    serial_workers,
    parallel_workers
FROM summary;